...
public class List {
    ...
    public int indexOf(int index, Object element){
        for (int i=index; i<positiveLength; i++){
            if (getElement(i).equals(element)){
                return i;
            }
        }
    ...
    public int reversIndexOf(Object element){
        for (int i=positiveLength-1; i>=~negativeLength+0b1; i--){
            if (getElement(i).equals(element)){
                return i;
            }
        }
    ...
    public int reversIndexOf(int index, Object element){
        for (int i=index; i>=~negativeLength+0b1; i--){
            if (getElement(i).equals(element)){
                return i;
            }
        }
    ...
    public void insert(Object element, int index){
        Object[] positiveInterm=new Object[positiveLength+1];
        Object[] negativeInterm=new Object[negativeLength+1];
        int counter=0;
        if (index<0){
            for (int i=0; i<=negativeLength; i++){
                if (-i-1==index){
                    negativeInterm[i]=element;
                    continue;
                }
                negativeInterm[i]=negativeList[counter];
                counter++;
            }
            negativeList=negativeInterm;
            negativeLength++;
        }
        else {
            for (int i=0; i<=positiveLength; i++){
                if (i==index){
                    positiveInterm[i]=element;
                    continue;
                }
                positiveInterm[i]=positiveList[counter];
                counter++;
            }
            positiveList=positiveInterm;
            positiveLength++;
        }
    }
